...
torch::Tensor ComputeRewardToGo(const torch::Tensor& rewards, double discount) {
  ...
  output[num_time_steps - 1] =
      pow(discount, num_time_steps - 1) * rewards[num_time_steps - 1];
...
torch::Tensor ComputeTemporalDifference(const torch::Tensor& critic_values,
  ...
  for (int32_t j = 0; j < num_agents; j++) {
    for (int32_t t = 0; t < num_time_steps - 1; t++) {
      temporal_difference[j][t] =
          rewards[j][t] + discount * critic_values[t + 1] - critic_values[t];
    }

    /* Handle the last time step (without discounting future value). */
    temporal_difference[j][num_time_steps - 1] =
        rewards[j][num_time_steps - 1] - critic_values[num_time_steps - 1];
  }
...
torch::Tensor
ComputeGeneralAdvantageEstimation(const torch::Tensor& temporal_differences,
  ...
  for (int32_t j = 0; j < num_agents; j++) {
    for (int32_t t = 0; t < num_time_steps; t++) {
      for (int32_t m = 0; m <= num_time_steps - t - 1; m++) {
        gae[j][t] +=
            pow(discount * gae_parameter, m) * temporal_differences[j][t + m];
      }
    }
  }
...
torch::Tensor
ComputeProbabilityRatio(const torch::Tensor& current_probabilities,
                        const torch::Tensor& previous_probabilities) {
  return current_probabilities.divide(previous_probabilities);
}

torch::Tensor
ComputePolicyLoss(const torch::Tensor& general_advantage_estimation,
                  const torch::Tensor& probability_ratio, float clip_value,
                  const torch::Tensor& policy_entropy) {
  /* Clip the probability ratio. */
  torch::Tensor probability_ratio_clipped =
      probability_ratio.clamp(1 - clip_value, 1 + clip_value);
  ...
  for (int32_t t = 0; t < num_time_steps; t++) {
    for (int32_t j = 0; j < num_agents; j++) {
      for (int32_t i = 0; i < mini_batch_size; i++) {
        loss += torch::min(probability_ratio[i][j][t] *
                               general_advantage_estimation[i][j][t],
...
torch::Tensor ComputeCriticLoss(const torch::Tensor& current_values,
                                const torch::Tensor& previous_values,
                                const torch::Tensor& reward_to_go,
                                float clip_value) {
  ...
  torch::Tensor current_values_clipped =
      torch::clamp(current_values, clipping_min, clipping_max);
...
torch::Tensor ComputePolicyEntropy(const torch::Tensor& actions_probabilities,
  ...
  torch::Tensor clipped_probabilities =
      torch::clamp(actions_probabilities, 1e-10, 1.0);
