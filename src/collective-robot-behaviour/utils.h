...
@param[in] critic_values: Values from the critic network per time step with the
shape [num_time_steps].
@param[in] rewards: Reward per time step per actor with the shape [num_agents,
num_time_steps].
@param[in] discount: Discount factor.
*/
...
/*!
@returns the probability ratio for all agents for each time step with the shape
...
num_agents, num_time_steps].
@param[in] previous_probabilities: Probability of choosing the same action for
each agent for each time step with previous policy, with shape [mini_batch_size,
num_agents, num_time_steps].
*/
torch::Tensor
...
@param[in] probability_ratio: Probability ratio for each agent for each chunk in
the mini batch with the shape [mini_batch_size, num_agents, num_time_steps].
