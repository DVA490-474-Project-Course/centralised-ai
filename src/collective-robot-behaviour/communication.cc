 ...
 * Description: Functions for communicating with the ssl interface and simulation interface.
...
{
  /* Important! Both ReceivePacket() and AnalyzeGameState() should ideally be on the same thread, but separate one. Not on the main thread as they are now.
     This is because ReceivePacket() is a blocking call and will wait for the next packet to arrive.
  */
