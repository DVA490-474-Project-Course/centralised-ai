...
{}
    ...
    MOCK_METHOD(ssize_t, recvfrom, (int sockfd, void* buf, size_t len, int
flags, sockaddr* src_addr, socklen_t* addrlen), (override));
};*/
...
protected:
  ...
  void SetUp() override {
    ...
    detection->set_t_sent(1235.0);    // Example timestamp
...
TEST_F(VisionClientDerived, InitializesCorrectly) {
  ...
  EXPECT_EQ(mock_client.GetClientAddress().sin_addr.s_addr,
            inet_addr("127.0.0.1"));
  EXPECT_GE(mock_client.GetSocket(), 0); // socket should be valid (>= 0)
}
...
TEST(VisionClientTest, HandlesEmptyPacket) {
  MockVisionClient mock_client("127.0.0.1", 10001);
  ...
  EXPECT_CALL(mock_client, ReceivePacket()).WillOnce(testing::Invoke([&]() {
  ...
  EXPECT_EQ(mock_client.GetRobotPositionX(0, centralised_ai::Team::kBlue),
            0.0f);
  EXPECT_EQ(mock_client.GetRobotPositionY(0, centralised_ai::Team::kBlue),
            0.0f);
...
TEST(VisionClientTest, HandlesMissingRobotOrientation) {
  ...
  EXPECT_EQ(mock_client.GetRobotPositionY(0, centralised_ai::Team::kBlue),
            100.0f);
  EXPECT_EQ(mock_client.GetRobotOrientation(0, centralised_ai::Team::kBlue),
            0.0f); // Check default orientation
}
...
TEST(VisionClientTest, HandlesMultipleRobotsAndBall) {
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    robot_blue->set_x(50.0f +
                      i * 10.0f); // Different x positions for each robot
    robot_blue->set_y(100.0f +
                      i * 5.0f); // Different y positions for each robot
  ...
  // Mock the behavior of ReceivePacket
  EXPECT_CALL(mock_client, ReceivePacket()).WillOnce(testing::Invoke([&]() {
    // Simulate processing the packet, handling multiple robots and a single
    // ball
    for (int i = 0; i < 3; ++i) {
      mock_client.SetBlueRobotPositionX(i, 50.0f + i * 10.0f);
      mock_client.SetBlueRobotPositionY(i, 100.0f + i * 5.0f);
      mock_client.SetBlueRobotOrientation(i,
                                          1.0f + i * 0.5f); // Default to zero
    }
  ...
  for (int i = 0; i < 3; ++i) {
    EXPECT_EQ(mock_client.GetRobotPositionX(i, centralised_ai::Team::kBlue),
              50.0f + i * 10.0f);
    EXPECT_EQ(mock_client.GetRobotPositionY(i, centralised_ai::Team::kBlue),
              100.0f + i * 5.0f);
    EXPECT_EQ(mock_client.GetRobotOrientation(i, centralised_ai::Team::kBlue),
              1.0f + i * 0.5f);
  }
