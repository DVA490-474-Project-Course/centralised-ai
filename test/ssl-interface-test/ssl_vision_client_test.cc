...
TEST_F(VisionClientDerived, TestReadVisionData) {
  mock_client.ReadVisionData(dummy_packet);

  /* Verify values after calling ReadVisionData */
  EXPECT_EQ(mock_client.GetTimestamp(), 1234);
  EXPECT_FLOAT_EQ(mock_client.GetRobotPositionX(1, centralised_ai::Team::kBlue),
      50.0f);//
  EXPECT_FLOAT_EQ(mock_client.GetRobotPositionY(1, centralised_ai::Team::kBlue),
      100.0f);//
  EXPECT_FLOAT_EQ(mock_client.GetRobotOrientation(1, centralised_ai::Team::kBlue),
      1.57f);//
